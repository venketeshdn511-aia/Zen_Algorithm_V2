import asyncio
import os
import sys
from motor.motor_asyncio import AsyncIOMotorClient
from datetime import datetime, timezone

# Add the project root to sys.path if needed
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

async def fix_token():
    # Use environment variables if available (inside docker)
    mongo_uri = os.environ.get("MONGODB_URI")
    if not mongo_uri:
        # Fallback to the one we know (AWS Production)
        mongo_uri = "mongodb+srv://venketeshdn511_db_user:VQ4NneK2loxp2uWz@zenalgo.2ihw5hx.mongodb.net/zen_algorithm?appName=ZenAlgo"
    
    token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..TvX0xrElKqpqoKWzmiAkFi-rlfATLHMhRsdfjUaRw0I"
    
    print(f"Connecting to MongoDB...")
    client = AsyncIOMotorClient(mongo_uri)
    db = client["tradedeck"]
    
    print(f"Updating fyers_access_token in 'config' collection...")
    await db.config.update_one(
        {"key": "fyers_access_token"},
        {"$set": {"value": token, "updated_at": datetime.now(timezone.utc)}},
        upsert=True
    )
    
    print("Verification:")
    doc = await db.config.find_one({"key": "fyers_access_token"})
    if doc and doc["value"] == token:
        print("✅ SUCCESS: Token updated and verified in MongoDB.")
    else:
        print("❌ FAILURE: Token mismatch or not found.")
        
    client.close()

if __name__ == "__main__":
    asyncio.run(fix_token())
